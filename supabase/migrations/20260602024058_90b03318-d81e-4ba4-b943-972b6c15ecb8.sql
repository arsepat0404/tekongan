...
DECLARE
  last_ts timestamptz;
BEGIN
  -- Truncate to 100 chars as a server-side safety net
  ...
  -- Per-player rate limit: max 1 message per 500ms (2 msgs/sec)
...
FOR EACH ROW EXECUTE FUNCTION public.enforce_chat_limits();